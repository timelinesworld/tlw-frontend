              ...
              <div style={{ width: "50%", display: "flex", alignItems: "center", justifyContent: "flex-end", paddingRight: "12px" }}>
                {ev.side === "negative" && (editingEventId === ev.id ? (
                  ...
                  >
                    ...
                    {isAdmin && (<button className="hover-btn" title="Delete" onClick={() => handleDeleteEvent(ev.id)} style={{ position: "absolute", top: "8px", left: "52px", background: "none", border: "none", cursor: "pointer", opacity: 0, transition: "opacity 0.2s", padding: "2px", display: "flex", alignItems: "center" }}><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#B83232" strokeWidth="1.5" strokeOpacity="0.6"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/><path d="M9 6V4a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2"/></svg></button>)}
                    <span style={{ fontFamily: "Georgia,serif", fontSize: "13px", fontWeight: 700, color: "#1C1C1E", display: "block", marginBottom: "5px" }}>{ev.title}</span>
                    {ev.details && ev.details.length > 0 ? (
                      <div style={{ display: "flex", flexDirection: "column", gap: "3px" }}>
                        {ev.details.map((d: string, di: number) => (<div key={di} style={{ fontFamily: "Arial,sans-serif", fontSize: "11px", color: "#555", lineHeight: 1.5 }}>{parseBold(d)}</div>))}
              ...
              <div style={{ width: "50%", display: "flex", alignItems: "center", justifyContent: "flex-start", paddingLeft: "12px" }}>
                ...
                {ev.side === "positive" && (editingEventId === ev.id ? (
                  ...
                  <div
                    ...
                    onMouseLeave={e => { e.currentTarget.querySelectorAll('.hover-btn').forEach((b: any) => b.style.opacity = '0'); }}
                  >
                    <button className="hover-btn" title="Copy" onClick={() => { const text = `${ev.year}${ev.title ? ' — ' + ev.title : ''}\n${ev.details ? ev.details.join('\n') : ev.description}`; navigator.clipboard.writeText(text); const btn = document.activeElement as HTMLElement; if (btn) btn.innerHTML = '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="20 6 9 17 4 12"/></svg>'; setTimeout(() => { if (btn) btn.innerHTML = '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#1A7A4A" strokeWidth="1.5" strokeOpacity="0.6"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>'; }, 2000); }} style={{ position: "absolute", top: "8px", right: "52px", background: "none", border: "none", cursor: "pointer", opacity: 0, transition: "opacity 0.2s", padding: "2px", display: "flex", alignItems: "center" }}>
                    ...
                    {isAdmin && (<button className="hover-btn" title="Edit" onClick={() => startEdit(ev)} style={{ position: "absolute", top: "8px", right: "30px", background: "none", border: "none", cursor: "pointer", opacity: 0, transition: "opacity 0.2s", padding: "2px", display: "flex", alignItems: "center" }}><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#1A7A4A" strokeWidth="1.5" strokeOpacity="0.6"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></button>)}
                    {isAdmin && (<button className="hover-btn" title="Delete" onClick={() => handleDeleteEvent(ev.id)} style={{ position: "absolute", top: "8px", right: "8px", background: "none", border: "none", cursor: "pointer", opacity: 0, transition: "opacity 0.2s", padding: "2px", display: "flex", alignItems: "center" }}><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#1A7A4A" strokeWidth="1.5" strokeOpacity="0.6"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/><path d="M9 6V4a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2"/></svg></button>)}
                    <span style={{ fontFamily: "Georgia,serif", fontSize: "13px", fontWeight: 700, color: "#1C1C1E", display: "block", marginBottom: "5px" }}>{ev.title}</span>
                    {ev.details && ev.details.length > 0 ? (
                      <div style={{ display: "flex", flexDirection: "column", gap: "3px" }}>
                        {ev.details.map((d: string, di: number) => (<div key={di} style={{ fontFamily: "Arial,sans-serif", fontSize: "11px", color: "#555", lineHeight: 1.5 }}>{parseBold(d)}</div>))}
